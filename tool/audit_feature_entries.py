...
def inventory():
    files = {p: p.read_text() for p in sorted((ROOT / 'lib').rglob('*.dart'))
             if 'generated' not in p.parts and 'l10n' not in p.parts and not p.name.endswith(GENERATED)}
    ...
    for p, source in files.items():
        ...
        module = rel.parts[4] if rel.parts[:4] == ('lib', 'src', 'presentation', 'pages') else '/'.join(rel.parts[1:3])
        modules[module] += 1
...
def render():
    ...
    out += ['', '测试目录分布：' + '、'.join(f'{name}: {count}' for name, count in sorted(tests.items())) + '。', '', '测试计数按目录归属，不把文件数当作行为覆盖率。', '', '## UI 构造入口索引', '',
            '| 模块 | 页面/组件 | 声明 | 外部构造引用 |', '|---|---|---|---|']
