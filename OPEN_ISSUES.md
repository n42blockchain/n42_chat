...
- Device operation follow-up: the connected Android passed the real-camera start/pause/resume smoke test. However, the host Flutter integration command omitted `--no-uninstall`; the tool defaults to uninstalling at teardown and calls `adb uninstall` without data preservation. The test installation and its local application data were removed without a preceding backup. The full-app debug build is ready, but both restore installs were blocked with `INSTALL_FAILED_USER_RESTRICTED`. The user has been informed of this mistake and asked to allow installation on the phone; restoration remains pending. Further device tests must use a dedicated installation and explicitly disable automatic uninstall when retaining data. See the wallet report `docs/testing/screenshot-regression-2026-09-16/README.md`.
...
- Registration gap (2026-09-15): the optional registration email is still not bound/verified by `MatrixAuthDataSource.register`; its recovery hint must not be treated as proof of a usable recovery address. Complete verified binding or remove that claim before release.
