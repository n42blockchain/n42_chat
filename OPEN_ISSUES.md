...
### STORAGE-001 Favorite deletion spans two preference keys — Fixed on audit branch, wallet integration pending
...
- Next step: update the wallet pin/mirror to the validated Chat audit branch revision. Until that happens, the wallet's pinned `c1d222e3` continues to contain the original two-key deletion. Retained legacy keys are migration input, not a downgrade synchronization mechanism; UI-001 remains separate.
...
### AUTH-001 Email verification contract — Client repair on audit branch; wallet integration, live acceptance and multi-email handling remain open
...
- Retry candidate validation: the complete plugin suite passes 6,411 tests with one credential-dependent test skipped; static analysis reports zero errors/warnings and 173 existing infos. A temporary wallet package configuration selecting the canonical candidate passes 498 Chat/quality checks. Evidence is archived in the wallet audit index under `EMAIL_RETRY_2026-09-14.md`; the audit branch repair still lacks live-account acceptance and a wallet dependency update.
- Next step: integrate the validated Chat audit branch revision into the wallet, then validate against an isolated account; implement an explicit multi-email management/primary-address policy before claiming complete email replacement. Do not remove recovery addresses implicitly. The wallet's current Git pin does not include this repair yet.
