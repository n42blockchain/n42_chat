...
void main() {
  ...
  group('Contact Remarks', () {
    ...
    test('should remove contact remark via setContactRemark with null',
        () async {
      await dataSource.setContactRemark('@alice:server.com', '小明');
    ...
    test('should remove contact remark via setContactRemark with empty string',
        () async {
      await dataSource.setContactRemark('@alice:server.com', '小明');
  ...
  group('Strong Reminders', () {
    test('should set and get strong reminder for a room', () async {
      ...
      final status =
          await dataSource.getStrongReminderStatus('!room1:server.com');
      expect(status, isTrue);
    });

    test('should return false for rooms without strong reminder', () async {
      final status =
          await dataSource.getStrongReminderStatus('!unknown:server.com');
  ...
  group('Deleted Messages', () {
    test('should mark messages as locally deleted', () async {
      ...
      final ids =
          await dataSource.getLocallyDeletedMessageIds('!room1:server.com');

      expect(ids, containsAll(['msg1', 'msg2', 'msg3']));
      expect(ids.length, equals(3));
    });

    test('should return empty set for rooms with no deleted messages',
        () async {
      final ids =
          await dataSource.getLocallyDeletedMessageIds('!unknown:server.com');
      expect(ids, isEmpty);
    });
    ...
    test('should keep messages separate per room', () async {
      ...
      final ids1 =
          await dataSource.getLocallyDeletedMessageIds('!room1:server.com');
      final ids2 =
          await dataSource.getLocallyDeletedMessageIds('!room2:server.com');
    ...
    test('should handle clearing messages for room with no data', () async {
      ...
      final ids = await dataSource
          .getLocallyDeletedMessageIds('!nonexistent:server.com');
      expect(ids, isEmpty);
    });
    ...
    test('should enforce 1000 message limit per room', () async {
      // Mark 1005 messages
      final messageIds =
          List.generate(1005, (i) => 'msg_${i.toString().padLeft(5, '0')}');
      await dataSource.markMessagesAsLocallyDeleted(
        '!room1:server.com',
        messageIds,
      );
  ...
  group('Destruction Times', () {
    ...
    test('should get all destruction times for a room', () async {
      ...
      await dataSource.setMessageDestroyedAt(
          '!room1:server.com', 'msg1', time1);
      await dataSource.setMessageDestroyedAt(
          '!room1:server.com', 'msg2', time2);

      final times =
          await dataSource.getMessageDestructionTimes('!room1:server.com');
    ...
    test('should return empty map for room with no destruction times',
        () async {
      final times =
          await dataSource.getMessageDestructionTimes('!unknown:server.com');
    ...
    test('should clear all destruction times for a room', () async {
      ...
      final times1 =
          await dataSource.getMessageDestructionTimes('!room1:server.com');
      final times2 =
          await dataSource.getMessageDestructionTimes('!room2:server.com');
    ...
    test('should handle clearing destruction times for non-existent room',
        () async {
      // Should not throw
      await dataSource
          .clearAllMessageDestructionTimes('!nonexistent:server.com');
      await dataSource.clearMessageDestructionTimes(
  ...
  group('Hidden Chats', () {
    ...
    test('should not duplicate chat IDs when hiding same chat twice',
        () async {
      await dataSource.hideChat('!room1:server.com');
      await dataSource.hideChat('!room1:server.com');
  ...
  group('Quick Replies', () {
    ...
    test('should remove a quick reply by ID', () async {
      ...
      final customReply =
          repliesBefore.firstWhere((r) => r['content'] == '临时回复');
    ...
    test('should update quick reply last used time', () async {
      ...
      final repliesBefore = await dataSource.getQuickReplies();
      final customReply =
          repliesBefore.firstWhere((r) => r['content'] == '测试回复');
      final customId = customReply['id'] as String;
      ...
      final repliesAfter = await dataSource.getQuickReplies();
      final updatedReply =
          repliesAfter.firstWhere((r) => r['id'] == customId);
      expect(updatedReply['lastUsed'], isNotNull);
    });

    test('should handle removing non-existent quick reply ID gracefully',
        () async {
  ...
  group('Translation Cache', () {
    test('should save and retrieve translation cache', () async {
      await dataSource.saveTranslationCache(
        'msg1',
        'en',
        'Hello World',
      );
    ...
    test('should return null for missing translation cache', () async {
      final result =
          await dataSource.getTranslationCache('msg_unknown', 'en');
      expect(result, isNull);
    });
    ...
    test('should enforce 500 entry size limit by evicting oldest', () async {
      ...
      final recentEntry =
          await dataSource.getTranslationCache('msg_509', 'en');
  ...
  group('Drafts', () {
    ...
    test('should store drafts per room independently', () async {
      ...
      expect(
          await dataSource.getDraft('!room1:server.com'), equals('草稿1'));
      expect(
          await dataSource.getDraft('!room2:server.com'), equals('草稿2'));
    });
  ...
  group('Chat Folders', () {
    test('should save and retrieve chat folders', () async {
      ...
      expect(
        (result[0]['roomIds'] as List).length,
        equals(2),
      );
    });
  ...
  group('Favorites', () {
    group('Favorite Messages', () {
      ...
      test('should overwrite favorites on save', () async {
        await dataSource.saveFavoriteMessages(jsonEncode([
          {'eventId': 'evt1'},
        ]));
        await dataSource.saveFavoriteMessages(jsonEncode([
          {'eventId': 'evt2'},
    ...
    group('Favorite Meta', () {
      test('should save and retrieve favorite meta JSON', () async {
        final metaJson = jsonEncode({
          'evt1': {'tags': ['work', 'important'], 'note': '重要消息'},
          'evt2': {'tags': ['personal'], 'note': ''},
        });
  ...
  group('Chat Background', () {
    test('should set and get chat background for a room', () async {
      await dataSource.setChatBackground(
          '!room1:server.com', '/images/bg1.png');

      final result =
          await dataSource.getChatBackground('!room1:server.com');
      expect(result, equals('/images/bg1.png'));
    });

    test('should return null when no chat background set', () async {
      final result =
          await dataSource.getChatBackground('!unknown:server.com');
      expect(result, isNull);
    });
    ...
    test('should keep per-room and default backgrounds separate', () async {
      await dataSource.setChatBackground(
          '!room1:server.com', '/images/room_bg.png');
      await dataSource.setDefaultChatBackground('/images/default_bg.png');
  ...
  group('Auto Download Settings', () {
    ...
    test('should return empty map when no settings saved', () async {
      final result = await dataSource.getAutoDownloadSettings();
      expect(result, isEmpty);
    });
  });
  ...
  group('Scheduled Messages', () {
    test('should save and retrieve scheduled messages', () async {
      ...
      final messages =
          await dataSource.getScheduledMessages('!room1:server.com');
    ...
    test('should return empty list for room with no scheduled messages',
        () async {
      final messages =
          await dataSource.getScheduledMessages('!unknown:server.com');
  ...
  group('Moment Settings', () {
    ...
    test('should save and get hidden moment users', () async {
      await dataSource.saveHiddenMomentUsers(
          ['@alice:server.com', '@bob:server.com']);
    ...
    test('should save and get blocked moment users', () async {
      await dataSource
          .saveBlockedMomentUsers(['@blocked:server.com']);
  ...
  group('Cross-feature Isolation', () {
    test('different features should not interfere with each other', () async {
      ...
      expect(await dataSource.getContactRemark('@alice:server.com'),
          equals('小明'));
      expect(await dataSource.getSetting('language'), equals('zh-CN'));
    });
