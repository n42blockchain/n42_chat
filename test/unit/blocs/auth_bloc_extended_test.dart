...
//   stub-only events (passkey / OTP), AuthErrorCleared,
...
void main() {
  ...
  group('AuthHomeserverCheckRequested', () {
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.checkHomeserver(any()))
      ...
      expect: () => [
        isA<AuthState>().having(
            (s) => s.homeserverStatus, 'hs', HomeserverStatus.checking),
        isA<AuthState>()
            .having(
                (s) => s.homeserverStatus, 'hs', HomeserverStatus.valid)
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        ...
        isA<AuthState>()
            .having(
                (s) => s.homeserverStatus, 'hs', HomeserverStatus.invalid)
  ...
  group('UpdateAvatar', () {
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.updateAvatar(any(), any()))
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        ...
        isA<AuthState>()
            ...
            .having(
                (s) => s.errorMessage, 'error', 'Avatar upload failed'),
      ],
    );
  ...
  group('UpdateUserProfile', () {
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.updateDisplayName(any()))
            .thenAnswer((_) async => true);
        when(() => mockAuthRepository.getUserProfileData())
            .thenAnswer((_) async => null);
        when(() => mockAuthRepository.currentUser).thenReturn(testUser);
        return buildBloc();
      },
      act: (bloc) =>
          bloc.add(const UpdateUserProfile(displayName: 'New Name')),
      expect: () => [
        isA<AuthState>()
            .having((s) => s.status, 'status', AuthStatus.loading),
        isA<AuthState>()
      ...
      verify: (_) {
        verify(() => mockAuthRepository.updateDisplayName('New Name'))
            .called(1);
      },
    );
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.updateUserProfileData(
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      act: (bloc) =>
          bloc.add(const UpdateUserProfile(gender: 'female')),
      expect: () => [
        isA<AuthState>()
            .having((s) => s.status, 'status', AuthStatus.loading),
        isA<AuthState>()
  ...
  group('AuthGetBoundEmailRequested', () {
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.getBoundEmail())
      ...
      expect: () => [
        isA<AuthState>()
            .having((s) => s.boundEmail, 'boundEmail', 'user@example.com'),
      ],
    );
  ...
  group('AuthTokenLoginRequested', () {
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockAuthRepository.loginWithToken(
      ...
      act: (bloc) => bloc.add(const AuthTokenLoginRequested(
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Unimplemented login methods — always emit error status
  // ─────────────────────────────────────────────────────────────────────────

  group('Unimplemented login methods emit error', () {
    blocTest<AuthBloc, AuthState>(
      'AuthPasskeyLoginRequested emits error state',
      build: buildBloc,
      act: (bloc) => bloc.add(
        const AuthPasskeyLoginRequested(homeserver: 'https://server.com'),
      ),
      expect: () => [
        isA<AuthState>()
            .having((s) => s.status, 'status', AuthStatus.error)
    ...
    blocTest<AuthBloc, AuthState>(
      'AuthRegisterPasskeyRequested emits error state',
      build: buildBloc,
      act: (bloc) => bloc.add(const AuthRegisterPasskeyRequested()),
      expect: () => [
        isA<AuthState>()
            .having((s) => s.status, 'status', AuthStatus.error)
            .having((s) => s.errorMessage, 'error', isNotNull),
    ...
    blocTest<AuthBloc, AuthState>(
      'AuthRequestEmailOtpRequested emits error state',
      build: buildBloc,
      act: (bloc) => bloc.add(const AuthRequestEmailOtpRequested(
        homeserver: 'https://server.com',
    ...
    blocTest<AuthBloc, AuthState>(
      'AuthEmailOtpLoginRequested emits error state',
      build: buildBloc,
      act: (bloc) => bloc.add(const AuthEmailOtpLoginRequested(
        homeserver: 'https://server.com',
        email: 'user@example.com',
        otp: '123456',
      )),
  ...
  group('AuthCheckBiometricAvailability', () {
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        ...
        when(() => mockSecureStorage.isBiometricEnabled())
            .thenAnswer((_) async => false);
        when(() => mockBiometricService.getBiometricTypeDescription())
      ...
      expect: () => [
        isA<AuthState>()
            .having(
                (s) => s.isBiometricAvailable, 'available', true)
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        isA<AuthState>()
            .having((s) => s.isBiometricEnabled, 'enabled', true),
      ],
    );
  });
  ...
  group('AuthBiometricLoginRequested', () {
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        ...
        when(() => mockSecureStorage.isBiometricEnabled())
            .thenAnswer((_) async => true);
        when(() => mockSecureStorage.getCredentials())
            .thenAnswer((_) async => null);
        return buildBloc();
      },
      act: (bloc) => bloc.add(const AuthBiometricLoginRequested()),
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        ...
        when(() => mockBiometricService.authenticate(
            ...
            )).thenAnswer((_) async => BiometricResult.failure(
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        ...
        isA<AuthState>()
            .having((s) => s.status, 'status', AuthStatus.error)
            .having(
                (s) => s.errorMessage, 'error', 'Invalid credentials'),
      ],
    );
  });
  ...
  group('AuthEnableBiometricLogin', () {
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        isA<AuthState>().having((s) => s.errorMessage, 'error',
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        ...
        when(() => mockSecureStorage.enableBiometricLogin(
              homeserver: any(named: 'homeserver'),
              username: any(named: 'username'),
            )).thenAnswer((_) async {});
        return buildBloc();
    ...
    blocTest<AuthBloc, AuthState>(
      ...
      expect: () => [
        isA<AuthState>()
            .having((s) => s.errorMessage, 'error', 'Too many attempts'),
      ],
    );
  });
  ...
  group('AuthDisableBiometricLogin', () {
    blocTest<AuthBloc, AuthState>(
      ...
      build: () {
        when(() => mockSecureStorage.disableBiometricLogin())
            .thenAnswer((_) async {});
        return buildBloc();
      },
